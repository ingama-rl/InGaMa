...
from jaxmarl.environments.coordinate import Coordinate, CoordinateLogWrapper
import pickle
...
def initialize_render(parameters, config, num_envs, key, width=7, show=False, env_actors=None):
    ...
    if show:
        ...
        render_window_kwargs = {'title':'Coordinate', 'figsize':(width, height)}
...
def run_single_step(x, unused, statics):
    ...
    def _aux(x):
        ...
        cont_return = cont_return + (1 - all_done).reshape(reward['player_1'].shape) * reward['player_1']
        comm_return = comm_return + (1 - all_done).reshape(reward['player_1_comm'].shape) * reward['player_1_comm']
...
def make_train(config):
    env = Coordinate(**config["ENV_KWARGS"])
    ...
    env = CoordinateLogWrapper(env, replace_info=True)
        ...
        def _update_step(update_runner_state, comm_update):
            ...
            def _update_epoch(update_state, unused):
                ...
                def _update_minbatch(train_states, batch_info):
                    ...
                    def _comm_actor_loss_fn(actor_params, init_hstate, traj_batch, gae, kl_coef, old_params):
                        # RERUN NETWORK
                        yell_ids = traj_batch.comm_action[..., 0] == 1
                        word_ids = traj_batch.comm_action[..., 1:].sum(axis=-1) > 0

                        _, pi = comm_actor_network.apply(
                            actor_params,
                            (init_hstate[0].squeeze(), init_hstate[1].squeeze()),
                            (traj_batch.comm_obs, traj_batch.communication, traj_batch.comm_done, yell_ids, word_ids),
                            True,
                        )
                        log_prob = pi.log_prob(traj_batch.comm_action)

                        # CALCULATE ACTOR LOSS
                        logratio = log_prob - traj_batch.comm_log_prob
                        ratio = jnp.exp(logratio)
                        gae = (gae - gae.mean()) / (gae.std() + 1e-8)
                        loss_actor1 = ratio * gae
                        loss_actor2 = (
                        ...
                        _, old_pi = comm_actor_network.apply(
                            old_params,
                            (init_hstate[0].squeeze(), init_hstate[1].squeeze()),
                            (traj_batch.comm_obs, traj_batch.communication, traj_batch.comm_done, yell_ids, word_ids),
                            False,
                        ...
                        old_pi = jax.lax.stop_gradient(old_pi)
                        kl = jax.jit(_kl_divergence_multicategorical)(old_pi, pi).mean()
                        # debug
                        clip_frac = jnp.mean(jnp.abs(ratio - 1) > config["CLIP_EPS_COMM"])

                        actor_loss = loss_actor - config["ENT_COEF_COMM"] * entropy + kl_coef * kl
                    ...
                    comm_actor_grad_fn = jax.value_and_grad(_comm_actor_loss_fn, has_aux=True)
                    comm_actor_loss, comm_actor_grads = comm_actor_grad_fn(
                        comm_actor_train_state.params, comm_ac_init_hstate, traj_batch, advantages, kl_coef_comm, old_train_states[1].params
                    )
                    ...
                    comm_mask = jnp.logical_or(_comm_update, config['SIMUL'] * jnp.ones_like(_comm_update))[0]


                    new_comm_actor_train_state = comm_actor_train_state.apply_gradients(grads=comm_actor_grads)
                    new_cont_actor_train_state = cont_actor_train_state.apply_gradients(grads=cont_actor_grads)
                    critic_train_state = critic_train_state.apply_gradients(grads=critic_grads)

                    comm_actor_train_state = comm_actor_train_state.replace(
                        step=comm_mask * new_comm_actor_train_state.step + (
                                1 - comm_mask) * comm_actor_train_state.step,
                        params=jax.tree.map(lambda nx, ox: comm_mask * nx + (1 - comm_mask) * ox,
                                            new_comm_actor_train_state.params, comm_actor_train_state.params),
                        opt_state=jax.tree.map(lambda nx, ox: comm_mask * nx + (1 - comm_mask) * ox,
                                               new_comm_actor_train_state.opt_state, comm_actor_train_state.opt_state),
                    )
                    ...
                    comm_loss = comm_actor_loss[0] + critic_loss[0]
                    ...
                    loss_info = {
                        ...
                        "comm_actor_loss": comm_actor_loss[0],
                        "value_loss": critic_loss[0],
                        "cont_entropy": cont_actor_loss[1][1],
                        "comm_entropy": comm_actor_loss[1][1],
                        "cont_ratio": cont_actor_loss[1][2],
                        "comm_ratio": comm_actor_loss[1][2],
                        "approx_cont_kl": cont_actor_loss[1][3],
                        "approx_comm_kl": comm_actor_loss[1][3],
                        "cont_clip_frac": cont_actor_loss[1][4],
                        "comm_clip_frac": comm_actor_loss[1][4],
                    }
                ...
                init_hstates = tuple([(ihs[0].reshape((1, config["NUM_ACTORS"], config["ATTENTION_LEN"], -1)), ihs[1].reshape((1, config["NUM_ACTORS"], -1))) for ihs in init_hstates])
                batch = (
                    init_hstates[0],
                    init_hstates[1],
                    init_hstates[2],
...
@hydra.main(version_base=None, config_path="config", config_name="ingama_coordinate")
def main(config):
