...
from jaxmarl.environments.multi_maze.utils_v2 import load_map, colors
from jaxmarl.environments.multi_maze.rendering_v2 import initiate_rendering, _render
...
class MultiMaze(MultiAgentEnv):
    def __init__(
        ...
        self.yell_cost = yell_cost
        self.word_cost = word_cost
        self.render_fps = render_fps
        self.render_mode = render_mode

        self.agent_range = jnp.arange(num_agents)
        self.cont_agents = [colors[i] + "_player" for i in range(num_agents)]
    ...
    def _random_agent_locs(self, key: chex.PRNGKey):
        ...
        # permutation = jax.random.permutation(subkey, len(self.valid_locs))
        agent_locs = []
        ...
        # return agent_locs[permutation], key
    ...
    def _random_doors_keys(self, key: chex.PRNGKey):
        ...
        decoy_keys = []
        cl_keys = []
        rs_rooms = [None] * len(self.keys_rooms)
        for i, kk in enumerate(self.keys_init):
            key, subkey = jax.random.split(key)
            kid = jax.random.choice(subkey, jnp.arange(len(kk)))
            keys.append(kk[kid].flatten())
            rs_rooms[self.colored_ids[i]] = self.keys_rooms[self.colored_ids[i]][kid]

        for i, clkk in enumerate(self.all_cl_keys):
            key, subkey = jax.random.split(key)
            clkid = jax.random.choice(subkey, jnp.arange(len(clkk)))
            cl_keys.append(clkk[clkid].flatten())
            rs_rooms[self.colorless_ids[i]] = self.keys_rooms[self.colorless_ids[i]][clkid]

        for i, dkk in enumerate(self.all_decoy_keys):
            key, subkey = jax.random.split(key)
            dkid = jax.random.choice(subkey, jnp.arange(len(dkk)))
            decoy_keys.append(dkk[dkid].flatten())
            rs_rooms[len(self.colored_ids) + len(self.colorless_ids) + i] = self.keys_rooms[len(self.colored_ids) + len(self.colorless_ids) + i][dkid]

        if self.decoy:
            keys.append(self.all_keys[-1])
            decoy_keys = jnp.stack(decoy_keys)
            decoy_id = jnp.where(door_perm == 0)[0][0]
        else:
            decoy_keys = jnp.array([])
            decoy_id = None

        keys = jnp.stack(keys)
        rs_rooms = jnp.array(rs_rooms, dtype=int)
        cl_keys = jnp.array(cl_keys, dtype=int)
        cl_keys = jnp.stack(cl_keys)

        return doors, self.all_cl_doors, keys[door_perm], rs_rooms, decoy_keys, cl_keys, decoy_id, key

    @partial(jax.jit, static_argnums=[0])
    def _random_goal(self, key: chex.PRNGKey):
        key, subkey = jax.random.split(key)
        gid = jax.random.choice(subkey, jnp.arange(len(self.all_goals)))
        goal = self.all_goals[gid]
        rs_goal = jnp.array([self.goal_rooms[gid]], dtype=int)
        return goal, rs_goal, key

    @partial(jax.jit, static_argnums=[0])
    def _get_rndid(self, key_rooms, goal_room):
        rid = 0
        # print('######################################')
        # print('key rooms len: ', len(key_rooms))
        # print('rs2rndid len: ', len(self.rs2rndid))
        for i, kr in enumerate(key_rooms):
            # print('i: ', i)
            # print('kr: ', kr)
            nrs = self.rs2rndid[i][rid]
            # print('nrs: ', nrs)
            rid = nrs[kr]
            # print('rid: ', rid)
            # print('---------------------')
        nrs = self.rs2rndid[-1][rid]
        # print('last nrs:', nrs)
        # print('goal_room: ', goal_room)
        rndid = nrs[goal_room[0]]
        # print('######################################')
        return rndid
    ...
    def initiate_maps(self, doors, cl_doors, keys, decoy_keys, cl_keys, goal, agent_locs, decoy_id):
        ...
        goal_map =  jnp.zeros((self.map_size, self.map_size), dtype=jnp.float32)
        goal_map = goal_map.at[goal[0], goal[1]].set(1)

        door_ind = jnp.arange(len(doors))
        key_ind = jnp.arange(len(keys))
        if self.decoy:
            all_doors = jnp.roll(doors, -decoy_id)[1:]
            door_ind = jnp.roll(door_ind, -decoy_id)[1:]
            all_keys = jnp.roll(keys, -decoy_id)
            all_keys = jnp.roll(jnp.concatenate((decoy_keys, all_keys[1:]), axis=0), -len(decoy_keys))
            key_ind = jnp.roll(key_ind, -decoy_id)
            key_ind = jnp.roll(jnp.concatenate((key_ind[0] * jnp.ones((len(decoy_keys),), dtype=int), key_ind[1:]), axis=0), -len(decoy_keys))
        else:
            all_doors = doors
            all_keys = keys
        ...
        if len(self.colorless_ids) > 0:
            for i in range(len(self.colorless_ids)):
                ...
                key_map = key_map.at[i + len(all_keys), key[0], key[1]].set(1)
                door_map = door_map.at[i + len(all_doors), door[0], door[1]].set(1)
    ...
    def observe_centralized(self, obs_state: ObsState):
        ...
        cl_door_view = door_view[len(self.all_doors):].sum(axis=0)[None, ...]
        door_view = jnp.concatenate((door_view[:len(self.all_doors)], cl_door_view), axis=0)
        ...
        return jnp.concatenate(
            (
                self.wall_map.flatten(),
                door_view.flatten(),
                key_view.flatten(),
                obs_state.goal_map.flatten(),
                obs_state.agent_map.flatten()
            ), axis=0
        )[None, :].repeat(self.num_agents, axis=0)
    ...
    def observe(self, obs_state: ObsState):
        def _observe(agent_id: jnp.ndarray, state: ObsState, wall_map: jnp.ndarray):
            ...
            return jnp.concatenate((wall_view, door_view, key_view, goal_view, agent_view), axis=0).flatten()
    ...
    def obs_to_map(self, obs):
        ...
        return {'agent_id': agent_one_hot, 'agent': agent_map, 'door': door_map, 'key': key_map, 'wall': wall_map, 'goal': goal_map}

    def comm_to_map(self, communication):
        chunked = jnp.split(communication, self.num_agents)
        res = []
        for chunk in chunked:
            ...
            wall_map = obs[..., self.object_locs['wall'][0]:self.object_locs['wall'][1]].reshape((1, self.map_size, self.map_size)).swapaxes(-1, -2)
            goal_map = obs[..., self.object_locs['goal'][0]:self.object_locs['goal'][1]].reshape((1, self.map_size, self.map_size)).swapaxes(-1, -2)
            res.append({'agent_id': agent_one_hot, 'agent': agent_map, 'door': door_map, 'key': key_map, 'wall': wall_map, 'goal': goal_map})
        return res

    @partial(jax.jit, static_argnums=[0])
    def human_comm(self, observations):
        def _human_comm(obs, aid):
            comm_action = jnp.zeros((len(self.vocab) + 1,))
            agent_obs = obs[self.object_locs['agent'][0]: self.object_locs['agent'][1]]
            agent_obs = jnp.roll(agent_obs, -aid * (self.map_size ** 2))

            comm_action = comm_action.at[self.txt2pos['agent'] + 1].set(jnp.any(agent_obs[self.map_size ** 2:]))
        ...
        vmap_human_comm = jax.vmap(_human_comm, (0, 0), 0)
        human_strategy = vmap_human_comm(observations, self.agent_range)
        return human_strategy

    @partial(jax.jit, static_argnums=[0])
    def prep_comm(self, comm_actions, last_observations):
        obj_masks = self.ca2mask[:len(self.object_words)]
        color_masks = self.ca2mask[len(self.object_words):]
        def _prep_comm(comm_action, last_obs):
            obj_mask = comm_action[1:len(self.object_words) + 1].reshape((1, -1)) @ obj_masks
            color_mask = comm_action[len(self.object_words) + 1:].reshape((1, -1)) @ color_masks
            mask = jnp.clip(obj_mask * color_mask, 0, 1).astype(jnp.float32)
    ...
    def get_rgdist(self, rndid, agent_locs):
        ...
        def _get_rgdist(aposes, dist):
            rid = 0
            # print("############################################")
            # print('len aposes: ', len(aposes))
            # print('len ar2rgid: ', len(self.ar2rgid))
    ...
    def reset(self, key: chex.PRNGKey) -> Tuple[chex.Array, State]:
        doors, cl_doors, keys, rs_rooms, decoy_keys, cl_keys, decoy_id, key = self._random_doors_keys(key)
        agent_locs, key = self._random_agent_locs(key)
        goal, rs_goal, key = self._random_goal(key)
        rndid = self._get_rndid(rs_rooms, rs_goal)
        doors_state = self.update_doors_state(agent_locs, jnp.concatenate((keys, cl_keys), axis=0))
        ...
        human_strategy = self.human_comm(obs_array)
        ...
        observations.update({'communication': proc_comm, 'centralized': cent_obs, 'human_strategy': human_strategy})
        # infos['success'] = {_rndid:
        #                         (-1) * (rndid != _rndid) * jnp.ones((self.num_agents,))
        #                         + (rndid == _rndid) * all_success * jnp.ones((self.num_agents,))
        #                     for _rndid in range(len(self.dists))}
        # infos['rgdist'] = {rndid:
        #                        (-1) * (state.rndid != rndid) * jnp.ones((self.num_agents,))
        #                        + (state.rndid == rndid) * rgdist * jnp.ones((self.num_agents,))
        #                    for rndid in range(len(self.dists))}
        state = State(
            n_steps=0,
            rgdist=rgdist,
    ...
    def validate_apply_action(self, actions, agent_locs, door_map, doors_state):
        def _validate_apply_action(loc, action, door_map, doors_state):
            ...
            is_wall = (jnp.any(new_loc >= self.map_size) + jnp.any(new_loc < 0) + self.wall_map[new_loc[0], new_loc[1]]) > 0
            door_oc = door_map[..., new_loc[0], new_loc[1]]
            door_oc = door_oc - doors_state.reshape(door_oc.shape) * door_oc
            is_wall = is_wall + door_oc.sum() > 0
            invalid = (is_wall + (new_loc < 0).any() + (new_loc >= self.map_size).any()) > 0
            return invalid * loc + (1 - invalid) * new_loc

        vmap_validate_apply_action = jax.vmap(_validate_apply_action, (0, 0, None, None), 0)
        new_locs = vmap_validate_apply_action(agent_locs, actions, door_map, doors_state)
        return new_locs
    ...
    def step_env(self, key: chex.PRNGKey, state: State, actions: dict):
        ...
        doors_state = self.update_doors_state(new_agent_locs, jnp.concatenate((state.keys, state.cl_keys), axis=0))
        obs_state = ObsState(
        ...
        cent_obs = self.observe_centralized(obs_state)
        human_strategy = self.human_comm(obs_array)
        raw_comm = self.prep_comm(comm_actions, state.observation)
        proc_comm = jnp.concatenate((raw_comm, comm_actions), axis=1).flatten()
        observations = {'communication': proc_comm, 'centralized': cent_obs, 'human_strategy': human_strategy}
        ...
        # reward = term1 * 1. + term2 * (1. - jnp.clip(rgdist,-1e8,1e8)) + term3 * (- state.rgdist)
        ...
        rewards, dones, infos = {}, {}, {}
        dones['__all__'] = jnp.logical_or(truncated, (self.max_episode_len >= jnp.inf) * all_success).astype(bool).reshape(())
        yells = comm_actions[:,0]
        yell_costs = self.yell_cost * yells.astype(jnp.float32)
        words = comm_actions[:,1:]
        n_words = jnp.sum(words, axis=1)
        word_costs = yells.flatten() * n_words.flatten() * self.word_cost
        infos['success'] = {rndid:
                                (-1) * (state.rndid != rndid) * jnp.ones((self.num_agents,))
                                + (state.rndid == rndid) * all_success * jnp.ones((self.num_agents,))
                            for rndid in range(len(self.dists))}
        infos['rgdist'] = {rndid:
                                (-1) * (state.rndid != rndid) * jnp.ones((self.num_agents,))
                                + (state.rndid == rndid) * (jnp.logical_or(term1, term2) * (jnp.clip(rgdist,-1e8,1e8)) + term3 * (state.rgdist + 1)) * jnp.ones((self.num_agents,))
                            for rndid in range(len(self.dists))}
        infos['truncated'] = truncated * jnp.ones(self.num_agents, dtype=bool)
        yell_info, hs_yell_info = {}, {}
        vocab_info = {word: {} for word in self.vocab}
        hs_vocab = {word: {} for word in self.vocab}
        for aid, agent in enumerate(self.cont_agents):
            dones[agent] = dones['__all__']
            dones[agent + '_comm'] = dones[agent]
            rewards[agent] = reward
            rewards[agent + '_comm'] = reward - yell_costs[aid] - word_costs[aid]
            observations[agent] = obs_array[aid]
            observations[agent + '_comm'] = obs_array[aid]
            word_pos = comm_actions[aid, 1:] * comm_actions[aid, 0]
            hs_word_pos = human_strategy[aid, 1:] * human_strategy[aid, 0]
            yell_info[agent] = jnp.array(comm_actions[aid, 0]).astype(jnp.float32)
            hs_yell_info[agent] = jnp.array(human_strategy[aid, 0]).astype(jnp.float32)
            for j, word in enumerate(self.vocab):
                vocab_info[word][agent] = jnp.array(word_pos[j]).astype(jnp.float32)
                hs_vocab[word][agent] = jnp.array(hs_word_pos[j]).astype(jnp.float32)
        infos['yell'] = yell_info
        infos['vocab'] = vocab_info
        infos['hs_vocab'] = hs_vocab
        infos['hs_yell'] = hs_yell_info
        new_state = State(
            n_steps=n_steps,
...
class LogEnvState:
    ...
    episode_lengths: int
    success_episode: Dict[int, jnp.ndarray]
    rndid_episode: jnp.ndarray
    avg_rgdist_episode: Dict[int, jnp.ndarray]
    returned_cont_episode_returns: jnp.ndarray
    returned_comm_episode_returns: jnp.ndarray
    returned_episode_lengths: jnp.ndarray
    returned_success_episode: Dict[int, jnp.ndarray]
    returned_rndid_episode: jnp.ndarray
    returned_avg_rgdist_episode: Dict[int, jnp.ndarray]
    ...
    episode_hs_yells: jnp.ndarray
    episode_hs_vocab: Dict[str, jnp.ndarray]
    returned_episode_hs_yells: jnp.ndarray
    returned_episode_hs_vocab: Dict[str, jnp.ndarray]
...
class MultiMazeLogWrapper(JaxMARLWrapper):
    ...
    def reset(self, key: chex.PRNGKey) -> Tuple[chex.Array, State]:
        ...
        state = LogEnvState(
            ...
            success_episode={rndid: jnp.zeros((self._env.num_agents,)) for rndid in range(len(self._env.dists))},
            rndid_episode=env_state.rndid * jnp.ones((self._env.num_agents,)),
            avg_rgdist_episode={rndid: jnp.zeros((self._env.num_agents,), dtype=jnp.float32) for rndid in range(len(self._env.dists))},
            ...
            returned_success_episode={rndid: jnp.zeros((self._env.num_agents,)) for rndid in range(len(self._env.dists))},
            returned_rndid_episode=jnp.zeros((self._env.num_agents,)),
            returned_avg_rgdist_episode={rndid: jnp.zeros((self._env.num_agents,), dtype=jnp.float32) for rndid in range(len(self._env.dists))},
            ...
            episode_hs_yells=jnp.zeros((self._env.num_agents,)),
            episode_hs_vocab={k: jnp.zeros((self._env.num_agents,)) for k in self._env.vocab},
            returned_episode_hs_yells=jnp.zeros((self._env.num_agents,)),
            returned_episode_hs_vocab={k: jnp.zeros((self._env.num_agents,)) for k in self._env.vocab},
        )
        return obs, state
    ...
    def step(
        ...
        new_avg_rgdist = {k: (state.avg_rgdist_episode[k] * state.episode_lengths + info['rgdist'][k]) * (1. / (state.episode_lengths + 1)) for k in range(len(self._env.dists))}
        ...
        new_success_episode = {k: info['success'][k] for k in range(len(self._env.dists))}
        ...
        new_episode_hs_yells = state.episode_hs_yells + self._batchify_floats(info['hs_yell'], self._env.cont_agents)
        new_episode_hs_vocab = {kk: state.episode_hs_vocab[kk] + self._batchify_floats(info['hs_vocab'][kk], self._env.cont_agents) for kk in self._env.vocab}

        new_state = LogEnvState(
            env_state=env_state,
            success_episode={k: v * (1 - ep_done) for k, v in new_success_episode.items()},
            rndid_episode= env_state.rndid * jnp.ones((self._env.num_agents,)),
            avg_rgdist_episode={k: v * (1 - ep_done) for k, v in new_avg_rgdist.items()},
            ...
            returned_success_episode={k: state.returned_success_episode[k] * (1 - ep_done)
                                         + new_success_episode[k] * ep_done
                                      for k in range(len(self._env.dists))},
            # returned_success_episode=state.returned_success_episode * (1 - ep_done)
            #                          + new_success_episode * ep_done,
            returned_rndid_episode=state.returned_rndid_episode * (1 - ep_done) + state.rndid_episode * ep_done,
            returned_avg_rgdist_episode={k: state.returned_avg_rgdist_episode[k] * (1 - ep_done) + new_avg_rgdist[k] * ep_done
                                         for k in range(len(self._env.dists))},
            ...
            episode_hs_yells=new_episode_hs_yells * (1 - ep_done),
            episode_hs_vocab={k: v * (1 - ep_done) for k, v in new_episode_hs_vocab.items()},
            returned_episode_hs_yells=state.returned_episode_hs_yells * (1 - ep_done)
                                      + new_episode_hs_yells * ep_done,
            returned_episode_hs_vocab={k: vp * (1 - ep_done) + vn * ep_done
                                       for k, vp, vn in
                                       zip(self._env.vocab,
                                           state.returned_episode_hs_vocab.values(),
                                           new_episode_hs_vocab.values())},
        )
        if self.replace_info:
            info = {}
        # print('len dists: ', len(self._env.dists))
        # print("rndid_episode: ", new_state.rndid_episode)
        # print("success_episode: ", new_state.success_episode)
        # print("rgdist_episode: ", new_state.avg_rgdist_episode)
        ...
        info["returned_success_episode"] = {str(k): v for k, v in new_state.returned_success_episode.items()}
        info["returned_rndid_episode"] = new_state.returned_rndid_episode
        info["returned_episode_avg_rgdist"] = {str(k): v for k, v in new_state.returned_avg_rgdist_episode.items()} #{str(k): v / new_state.returned_episode_lengths
                                               # for k, v in new_state.returned_avg_rgdist_episode.items()}
        ...
        info["returned_episode_hs_yells"] = new_state.returned_episode_hs_yells / new_state.returned_episode_lengths
        info["returned_episode_hs_vocab"] = {
            k: v / new_state.returned_episode_lengths for k, v in new_state.returned_episode_hs_vocab.items()
        }
...
def example():
    ...
    env = make('multi_maze', num_agents=3, max_episode_len=50, map='Triple Trouble')

    obs, state = env.reset(key)
    env.render(state, render_mode='human')
    ...
    for _ in range(20):
        ...
        key_act = jax.random.split(key_act, 2*env.num_agents)
        actions = {
        ...
        print("cont reward:", reward["red_player"])
        print("comm reward:", reward["red_player_comm"])
