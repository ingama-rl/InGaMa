from jaxmarl.environments.coordinate.coordinate import WAIT, LEFT, RIGHT, MARK, CLAIM, CoordinateLogWrapper
...
with jax.disable_jit():
    width = 8
    env = CoordinateLogWrapper(make('coordinate', grid_size=6, num_agents=2, max_episode_len=30), replace_info=True)
    ...
    render_window = Window('Coordinate', figsize=(width, height))
    render_window.show(block=False)
    render_window.show_img(_curr_view)

    comm_act = env.action_space('player_1_comm').sample(key_act)
    cont_act = env.action_space('player_1').sample(key_act2)
